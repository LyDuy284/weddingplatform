...
public class RejectBookingDetail {
    public static final String content(RejectMailDTO content) {
        ...
        emailBody.append(
                "<p><b>The Day</b> xin thông báo bạn có dịch vụ đã bị từ chối bởi <b>" + content.getSupplierName()
                        + "</b>.</p>");
        emailBody.append(
        ...
        emailBody.append("<span style=\"width: 8%; text-align: center;\"><b>" + content.getBookingDetail().getQuantity()
                + "</b></span>");
        emailBody.append(
                "<span style=\"width: 17%; text-align: right;\">" + content.getBookingDetail().getPrice()
                        + "</span></div>");
        ...
        if (content.getListCurrentBookingDetails().size() > 0) {
            ...
            emailBody.append(
                    "<div style=\"border: 2px black solid; width: 80%; padding: 1.8rem; margin: auto;  border-radius: 0.6rem;\">");
            emailBody.append("<h3 style=\"margin: 0; text-align: center;\">The Day Wedding Platform</h3>");
            emailBody.append("<p>Mã Đơn Hàng: <b>" + content.getBookingDetail().getBooking().getId() + "</b></p>");
            emailBody.append(
                    "<p>Ngày tạo: <b><i>" + content.getBookingDetail().getBooking().getCreatedAt() + "</i></b></p>");
            ...
            for (BookingDetail bookingDetail : content.getListCurrentBookingDetails()) {
                ...
                emailBody
                        .append("<span style=\"width: 8%; text-align: center;\"><b>" + bookingDetail.getQuantity()
                                + "</b></span>");
                emailBody.append(
                        "<span style=\"width: 17%; text-align: right;\">" + bookingDetail.getPrice() + "</span></div>");
                emailBody
