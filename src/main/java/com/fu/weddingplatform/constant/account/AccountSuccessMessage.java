package com.fu.weddingplatform.constant.account;

public final class AccountSuccessMessage {
    public static final String GET_ALL_ACCOUNT = "Get all accounts successfully";
    public static final String GET_COUPLE = "Get couple profile successfully";
    public static final String GET_SUPPLIER = "Get supplier profile successfully";
    public static final String CREATE_SUCCESS = "Register account successfully.";
    public static final String CHECK_EXIST = "Account is existed";
    public static final String GET_ALL_ACTIVATE_ACCOUNT = "Get all activate accounts successfully";
    public static final String ACTIVATE_ACCOUNT = "Activate account successfully";
    public static final String DISABLE_ACCOUNT = "Disable account successfully";
    public static final String UPDATE_ACCOUNT = "update account successfully";

}
